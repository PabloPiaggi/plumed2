...
//+PLUMEDOC VES_TARGETDIST TD_MULTIBARIC
/*
Multibaric target distribution (dynamic).


*/
//+ENDPLUMEDOC

class TD_Multibaric: public TargetDistribution {
private:
  double threshold_, min_press_, max_press_, press_;
  double sigma_min_, sigma_max_;
public:
  static void registerKeywords(Keywords&);
  explicit TD_Multibaric(const ActionOptions& ao);
  void updateGrid();
...
PLUMED_REGISTER_ACTION(TD_Multibaric,"TD_MULTIBARIC")


void TD_Multibaric::registerKeywords(Keywords& keys) {
  TargetDistribution::registerKeywords(keys);
  keys.add("compulsory","THRESHOLD","Maximum exploration energy in KT.");
  keys.add("compulsory","MIN_PRESS","Minimum energy of the multicanonical ensemble.");
  keys.add("compulsory","MAX_PRESS","Maximum energy of the multicanonical ensemble.");
  keys.add("compulsory","PRESSURE","Pressure");
...
TD_Multibaric::TD_Multibaric(const ActionOptions& ao):
  PLUMED_VES_TARGETDISTRIBUTION_INIT(ao),
  threshold_(10.0),
  min_press_(0.0000602),
  max_press_(0.602)
{
  ...
  parse("MIN_PRESS",min_press_);
  parse("MAX_PRESS",max_press_);
...
double TD_Multibaric::getValue(const std::vector<double>& argument) const {
  plumed_merror("getValue not implemented for TD_Multibaric");
  return 0.0;
}


void TD_Multibaric::updateGrid() {
  if (getStep() == 0) {
    // Use uniform TD
    ...
    plumed_massert(getFesGridPntr()!=NULL,"the FES grid has to be linked to use TD_Multibaric!");
 
    // Find minimum of F(U) at minimum pressure
    ...
    for(Grid::index_t l=0; l<targetDistGrid().getSize(); l++) {
      ...
      value = beta*value + beta*(min_press_ - press_)*argument;
    ...
    for(Grid::index_t l=minindex; l>1; l-=1) {
      ...
      value = beta*value + beta*(min_press_ - press_)*argument - minval;
      value_next = beta*value_next + beta*(min_press_ - press_)*argument_next - minval;
    ...
    for(Grid::index_t l=0; l<targetDistGrid().getSize(); l++) {
      double value = getFesGridPntr()->getValue(l);
      double argument = targetDistGrid().getPoint(l)[0];
      value = beta*value + beta*(max_press_ - press_)*argument;
      //log.printf(" l arg val %d %f %f \n",l,argument,value);
      if(value<minval) {
        minval=value;
        minpos=argument;
    ...
    for(Grid::index_t l=minindex; l>1; l-=1) {
      ...
      double value = getFesGridPntr()->getValue(l);
      double value_next = getFesGridPntr()->getValue(l-1);
      value = beta*value + beta*(max_press_ - press_)*argument - minval;
      value_next = beta*value_next + beta*(max_press_ - press_)*argument_next - minval;
      if (value<threshold_ && value_next>threshold_) {
...
inline
double TD_Multibaric::GaussianSwitchingFunc(const double argument, const double center, const double sigma) const {
  if(sigma>0.0) {
